...
import com.zeroitsolutions.ziloo.Adapters.HashTagFavouriteAdapter;
...
public class SearchHashTagsF extends RootFragment {
    ...
    public void callApiSearch() {
        ...
        try {
            if (Functions.getSharedPreference(context).getString(Variable.U_ID, null) != null) {
                params.put("user_id", Functions.getSharedPreference(context).getString(Variable.U_ID, "0"));
            }
        ...
        ApiVolleyRequest.JsonPostRequest(getActivity(), ApiLinks.search, params, Functions.getHeaders(getActivity()), new InterfaceApiResponse() {
            @Override
    ...
    public void callApiGetFavourite() {
        ...
        ApiVolleyRequest.JsonPostRequest(getActivity(), ApiLinks.showFavouriteHashtags, params, Functions.getHeaders(getActivity()), new InterfaceApiResponse() {
            @Override
            public void onResponse(String resp) {
                Functions.checkStatus(getActivity(),resp);
                shimmerFrameLayout.stopShimmer();
