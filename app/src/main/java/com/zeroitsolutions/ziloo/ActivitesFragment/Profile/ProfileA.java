...
import com.zeroitsolutions.ziloo.ActivitesFragment.Profile.FollowTab.NotificationPriorityF;
...
import com.zeroitsolutions.ziloo.Adapters.SuggestionAdapter;
...
public class ProfileA extends AppCompatActivity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Functions.setLocale(Functions.getSharedPreference(ProfileA.this).getString(Variable.APP_LANGUAGE_CODE, Variable.DEFAULT_LANGUAGE_CODE)
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.tvFollowBtn:
                if (tvFollowBtn.getText().toString().equalsIgnoreCase(getString(R.string.message))) {
                    openChatF();
                } else {
                    if (Functions.checkLoginUser(ProfileA.this))
                        followUnFollowUser();
            ...
            case R.id.message_btn:
                if (Functions.getSharedPreference(context).getString(Variable.U_ID, "").equals(userId)) {
                    openSettingScreen();
                } else {
                    showVideoOption();
                }
    ...
    private void showVideoOption() {
        ...
        if (blockByUserId.equals(Functions.getSharedPreference(context).getString(Variable.U_ID, ""))) {
    ...
    private void openProfileShareTab() {
        ...
        if (userId.equalsIgnoreCase(Functions.getSharedPreference(ProfileA.this).getString(Variable.U_ID, ""))) {
            fromSetting = true;
        } else {
            fromSetting = false;
        }
    ...
    private void getSuggestionUserList() {
        ...
        ApiVolleyRequest.JsonPostRequest(ProfileA.this, ApiLinks.showSuggestedUsers, parameters, Functions.getHeaders(this), new InterfaceApiResponse() {
            @Override
    ...
    private void setupProfileIcon() {
        if (userId.equalsIgnoreCase(Functions.getSharedPreference(ProfileA.this).getString(Variable.U_ID, "0"))) {
    ...
    private void openBlockUserDialog() {
        ...
        try {
            params.put("user_id",
                    Functions.getSharedPreference(ProfileA.this).getString(Variable.U_ID, ""));
        ...
        ApiVolleyRequest.JsonPostRequest(ProfileA.this, ApiLinks.blockUser, params, Functions.getHeaders(this), new InterfaceApiResponse() {
            @Override
            public void onResponse(String resp) {
                Functions.checkStatus(ProfileA.this, resp);
                Functions.cancelLoader();
