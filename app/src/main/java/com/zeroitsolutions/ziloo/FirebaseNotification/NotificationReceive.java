package com.zeroitsolutions.ziloo.FirebaseNotification;
...
public class NotificationReceive extends FirebaseMessagingService {
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        if (remoteMessage.getData().size() > 0) {
            ...
            if (pic != null && !pic.contains(Variable.http))
                pic = Constants.BASE_MEDIA_URL + pic;

            senderid = remoteMessage.getData().get("user_id");
            receiverid = sharedPreferences.getString(Variable.U_ID, "");
    ...
    public void onNewToken(String s) {
        ...
        if (s == null) {
            ...
            sharedPreferences.edit().putString(Variable.DEVICE_TOKEN, s).commit();
        }
    }
    ...
    public void chatFragment(String senderid, String name, String picture) {
        ...
        if (sharedPreferences.getBoolean(Variable.IS_LOGIN, false)) {
    ...
    private class SendNotification extends AsyncTask<String, Void, Bitmap> {
        ...
        protected void onPostExecute(Bitmap result) {
            ...
            // its for multiple account notification handling
            if (Functions.getSharedPreference(getApplicationContext()).getString(Variable.U_ID, "").equalsIgnoreCase(userId)) {
